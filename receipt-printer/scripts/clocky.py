...
def print_to_printer(output_line, include_date=False):
    ...
    try:
        # Prepend the date and time if required
        if include_date:
            output_line = f"Today is {get_current_datetime()}\n\n{output_line}"

        # Calculate the number of content lines (32 characters per line)
        content_lines = (len(output_line) + 31) // 32  # Ceiling division

        # Add three header lines
        header = "\n" * 3

        # Calculate the total lines printed so far (content + header)
        total_lines = content_lines + 3

        # Calculate the padding needed to make the receipt 18 lines
        if total_lines < 18:
